    ...
    return (
        <div className='bg-light-gray'>
            <div className="container mx-auto max-w-[1535px] sm:p-[30px] py-6 px-4 ">
                ...
                <div className={`flex flex-col border border-dark-blue/10 rounded-2xl overflow-hidden  mt-[30px] ${notifications.length === 0 && 'hidden'}`}>
                    {notifications.map((item, index) => (
                        <div key={index} className={`flex items-center justify-between py-2 sm:py-4 px-3 sm:px-6 gap-3 border-b hover:bg-light-blue transition-all duration-300 border-dark-blue/10 ${index === notifications.length - 1 ? 'border-b-0' : ''} `}>
                            ...
                            <CustomButton customOnClick={() => handleClearNotification(index)} iconOne='deleteIcon' buttonTextTwo='Clear' buttonClass='border-0 text-dark-black max-sm:px-4 ' iconOneClass={'stroke-dark-black'} buttonClassTwo={'max-sm:hidden'} />
                        </div>
                    ))}
                </div>
