...
const DashBoardData = () => {
  return (
    <div className="bg-light-gray flex w-full flex-col gap-[30px] p-[30px]">
      <div className="min-h-[227px] overflow-hidden shadow-layer rounded-2xl bg-cover px-[55px] max-lg:px-5 max-2xl:px-10 items-center justify-between flex bg-center bg-[url('/assets/images/dashboard-image-layer.webp')]">
        <div className="flex flex-col justify-center">
          <p className="md:text-xl leading-100 text-white/80 mb-1.5">September 6, 2023</p>
          <h3 className="text-custom-4xl max-2xl:text-4xl max-xl:text-3xl max-lg:text-2xl max-md:text-xl max-sm:text-lg sm:whitespace-nowrap leading-130 mb-2 font-semibold text-light-white">Welcome, Rajpal Singh!</h3>
      ...
      <div className="w-full flex gap-6 flex-wrap">
        {STUDENTS_LIST.map((item, index) => (
          <div className="max-w-[496px] max-xl:flex-col max-sm:flex-col max-lg:flex-row max-[921px]:flex-row max-xl:max-w-[unset] max-xl:w-[unset] hover:shadow-student transition-all duration-300 w-full border border-solid border-light-gray-blue p-5 items-center rounded-2xl flex gap-[30px]" key={index}>
            <Image src={item.image} width={226} height={218} alt="student-image" className="w-[226px] h-[218px] object-cover rounded-2xl" />
